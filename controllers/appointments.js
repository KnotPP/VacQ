...
exports.getAppointment=async(req,res,next)=>{
  try{
    ...
    if(!appointment){
      return res.status(404).json({success:false,message:`No message with the id of ${req.params.id}`})
    }
...
exports.addAppointment=async(req,res,next)=>{
  try{
    ...
    if(!hospital){
      return res.status(404).json({success:false, message:`No Hospital with the id of ${req.params.id}`})
    }
